...
parser.add_argument("--data", "-d", default=None, type=str, required=True,
                    help="The input data file, e.g., 'data/xnli.tsv'.")
...
json.dump(new_sentence1s, open(
    Path(args.output, 'xnli-' + args.split + '-sentence1-reference-translations-' + args.matrix + '-head.json'), 'w'),
          indent=False, ensure_ascii=False)
json.dump(new_sentence2s, open(
    Path(args.output, 'xnli-' + args.split + '-sentence2-reference-translations-' + args.matrix + '-head.json'), 'w'),
          indent=False, ensure_ascii=False)
